import React from "react";
import TablePagination from "./TablePagination";
import MainTable from "../MainTable";
import TablePaginationControls from "./TablePaginationControls";
var meta = {
  component: TablePagination,
  tags: ["autodocs"],
  argTypes: {
    totalItems: {
      if: {
        arg: "externallyControlled",
        truthy: true
      }
    },
    currentPage: {
      if: {
        arg: "externallyControlled",
        truthy: true
      }
    },
    pageSize: {
      if: {
        arg: "externallyControlled",
        truthy: true
      }
    },
    onPageChange: {
      if: {
        arg: "externallyControlled",
        truthy: true
      }
    },
    onPageSizeChange: {
      if: {
        arg: "externallyControlled",
        truthy: true
      }
    }
  }
};
export default meta;
export var Default = {
  name: "Default",
  args: {
    data: [{
      id: "row-1"
    }, {
      id: "row-2"
    }, {
      id: "row-3"
    }, {
      id: "row-4"
    }, {
      id: "row-5"
    }]
  }
};
export var CustomPageLimit = {
  name: "CustomPageLimit",
  args: {
    data: [{
      id: "row-1"
    }, {
      id: "row-2"
    }, {
      id: "row-3"
    }, {
      id: "row-4"
    }, {
      id: "row-5"
    }],
    pageLimits: [1, 2, 3]
  }
};
export var CustomDisplayTitle = {
  name: "CustomDisplayTitle",
  args: {
    data: [{
      id: "row-1"
    }, {
      id: "row-2"
    }, {
      id: "row-3"
    }, {
      id: "row-4"
    }, {
      id: "row-5"
    }],
    description: /*#__PURE__*/React.createElement("b", null, "Hello there")
  }
};
export var RenderAbove = {
  render: () => {
    var data = [{
      columns: [{
        content: "Ready",
        role: "rowheader"
      }, {
        content: 1,
        className: "u-align--right"
      }, {
        content: "1 GiB",
        className: "u-align--right"
      }, {
        content: 2,
        className: "u-align--right"
      }, {
        content: 42,
        className: "u-align--right"
      }],
      sortData: {
        status: "ready",
        cores: 2,
        ram: 1,
        disks: 2
      }
    }, {
      columns: [{
        content: "Idle",
        role: "rowheader"
      }, {
        content: 1,
        className: "u-align--right"
      }, {
        content: "1 GiB",
        className: "u-align--right"
      }, {
        content: 2,
        className: "u-align--right"
      }, {
        content: 23,
        className: "u-align--right"
      }],
      sortData: {
        status: "idle",
        cores: 1,
        ram: 1,
        disks: 2
      }
    }, {
      columns: [{
        content: "Waiting",
        role: "rowheader"
      }, {
        content: 8,
        className: "u-align--right"
      }, {
        content: "3.9 GiB",
        className: "u-align--right"
      }, {
        content: 3,
        className: "u-align--right"
      }, {
        content: 0,
        className: "u-align--right"
      }],
      sortData: {
        status: "waiting",
        cores: 8,
        ram: 3.9,
        disks: 3
      }
    }];
    var headers = [{
      content: "Status",
      sortKey: "status"
    }, {
      content: "Cores",
      sortKey: "cores",
      className: "u-align--right"
    }, {
      content: "RAM",
      sortKey: "ram",
      className: "u-align--right"
    }, {
      content: "Disks",
      sortKey: "disks",
      className: "u-align--right"
    }, {
      content: "Networks",
      className: "u-align--right"
    }];
    return /*#__PURE__*/React.createElement(TablePagination, {
      data: data,
      pageLimits: [1, 2, 3]
    }, /*#__PURE__*/React.createElement(MainTable, {
      headers: headers,
      rows: data,
      sortable: true
    }));
  },
  name: "RenderAbove"
};
export var RenderBelow = {
  render: () => {
    var data = [{
      columns: [{
        content: "Ready",
        role: "rowheader"
      }, {
        content: 1,
        className: "u-align--right"
      }, {
        content: "1 GiB",
        className: "u-align--right"
      }, {
        content: 2,
        className: "u-align--right"
      }, {
        content: 42,
        className: "u-align--right"
      }],
      sortData: {
        status: "ready",
        cores: 2,
        ram: 1,
        disks: 2
      }
    }, {
      columns: [{
        content: "Idle",
        role: "rowheader"
      }, {
        content: 1,
        className: "u-align--right"
      }, {
        content: "1 GiB",
        className: "u-align--right"
      }, {
        content: 2,
        className: "u-align--right"
      }, {
        content: 23,
        className: "u-align--right"
      }],
      sortData: {
        status: "idle",
        cores: 1,
        ram: 1,
        disks: 2
      }
    }, {
      columns: [{
        content: "Waiting",
        role: "rowheader"
      }, {
        content: 8,
        className: "u-align--right"
      }, {
        content: "3.9 GiB",
        className: "u-align--right"
      }, {
        content: 3,
        className: "u-align--right"
      }, {
        content: 0,
        className: "u-align--right"
      }],
      sortData: {
        status: "waiting",
        cores: 8,
        ram: 3.9,
        disks: 3
      }
    }];
    var headers = [{
      content: "Status",
      sortKey: "status"
    }, {
      content: "Cores",
      sortKey: "cores",
      className: "u-align--right"
    }, {
      content: "RAM",
      sortKey: "ram",
      className: "u-align--right"
    }, {
      content: "Disks",
      sortKey: "disks",
      className: "u-align--right"
    }, {
      content: "Networks",
      className: "u-align--right"
    }];
    return /*#__PURE__*/React.createElement(TablePagination, {
      data: data,
      pageLimits: [1, 2, 3],
      position: "below"
    }, /*#__PURE__*/React.createElement(MainTable, {
      headers: headers,
      rows: data,
      sortable: true
    }));
  },
  name: "RenderBelow"
};

/** The table pagination controls can be used without wrapping MainTable by
 * using the `TablePaginationControls` component.
 */
export var ControlsOnly = {
  render: () => {
    return /*#__PURE__*/React.createElement(TablePaginationControls, {
      currentPage: 1,
      itemName: "row",
      nextButtonProps: {
        disabled: false
      },
      onInputPageChange: console.log,
      onNextPage: console.log,
      onPageSizeChange: console.log,
      onPreviousPage: console.log,
      pageLimits: [10, 25, 50],
      pageSize: 20,
      previousButtonProps: {
        disabled: false
      },
      showPageInput: true,
      totalItems: 100,
      visibleCount: 10
    });
  }
};

/** The `TablePaginationControls` component can be used when the total
 *  number of entries is unknown.
 */
export var ControlsWithUnknownEntries = {
  render: () => {
    return /*#__PURE__*/React.createElement(TablePaginationControls, {
      currentPage: 1,
      itemName: "row",
      nextButtonProps: {
        disabled: false
      },
      onInputPageChange: console.log,
      onNextPage: console.log,
      onPageSizeChange: console.log,
      onPreviousPage: console.log,
      pageLimits: [10, 25, 50],
      pageSize: 20,
      previousButtonProps: {
        disabled: false
      },
      showPageInput: true,
      visibleCount: 10
    });
  }
};

/** The `TablePaginationControls` component can be used when it is known
 *  that there are more entries than the amount displayed on the current page.
 */
export var ControlsWithPartiallyKnownEntries = {
  render: () => {
    return /*#__PURE__*/React.createElement(TablePaginationControls, {
      currentPage: 3,
      itemName: "row",
      nextButtonProps: {
        disabled: false
      },
      onInputPageChange: console.log,
      onNextPage: console.log,
      onPageSizeChange: console.log,
      onPreviousPage: console.log,
      pageLimits: [10, 25, 50],
      pageSize: 20,
      previousButtonProps: {
        disabled: false
      },
      showPageInput: true,
      visibleCount: 10
    });
  }
};